...
!
      ...
      subroutine pnetcdf_write(filename)
          ...
          if (method .EQ. 1) then  ! using nonblocking APIs
            ...
            buffer = 2048000000
            err = nfmpi_buffer_attach(ncid, buffer)
            if (err .ne. NF_NOERR) call handle_err('buffer not created', err)
              subsizes(4) = nsc + 1
              
              err = nfmpi_bput_vara_double(ncid, yspecies_id, starts, subsizes, yspecies, req(1))
              if (err .ne. NF_NOERR) call handle_err('nfmpi_iput_vara_double on yspecies', err)

              !---- write array u
              subsizes(4) = 3
              err = nfmpi_bput_vara_double(ncid, u_id, starts, subsizes, u, req(2))
