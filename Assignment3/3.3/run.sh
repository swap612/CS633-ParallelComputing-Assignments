...
do
    ...
    rm -f /scratch/$USER/*
    mpirun -machinefile hostfile -np $N ./s3d_io.x 200 400 400 2 4 4 1 F /scratch/$USER $nsc
    for times in {1..10}
    do
            mpirun -machinefile hostfile -np $N ./s3d_io.x 200 400 400 2 4 4 1 T /scratch/$USER $nsc > run.txt
    ...
    echo "Running for N = $N"
    rm -f /scratch/$USER/*
    mpirun -machinefile hostfile -np $N ./s3d_io.x 400 400 800 4 4 8 1 F /scratch/$USER $nsc
    for times in {1..10}
    do
            mpirun -machinefile hostfile -np $N ./s3d_io.x 400 400 800 4 4 8 1 T /scratch/$USER $nsc > run.txt
            cat run.txt >> output.txt
            cat run.txt | grep 'read  bandwidth' | awk -F ":" '{print $2}' | sed 's/[MiB/s]/ /g; s/ //g' >readfile
            cat run.txt | grep 'write bandwidth' | awk -F ":" '{print $2}' | sed 's/[MiB/s]/ /g; s/ //g' >writefile
            awk -v nsc="$nsc" -v n="$N" '{print nsc"_"n ", "$0}' readfile  >> read_plot
